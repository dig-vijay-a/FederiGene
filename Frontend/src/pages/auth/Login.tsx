...
export default function Login() {
    ...
    const handleInputChange = (e) => {
        setFormData(prev => ({ ...prev, [(e.target as any).name]: (e.target as any).value }));
    };
    ...
    return (
        <div className="auth-container">
            <div className="glass-card auth-card">
                ...
                <p className="auth-subtitle">{step === 5 ? 'Verify your identity using your security questions.' : 'Secure researcher access.'}</p>
                ...
                {step === 2 && (
                    <form onSubmit={handle2FA} className="auth-form">
                        <input type="text" placeholder="000000" required maxLength="6" value={totpCode} onChange={e => setTotpCode((e.target as any).value)} className="totp-input" autoFocus />
                ...
                {step === 5 && (
                    <form onSubmit={handleFallbackSubmit} className="auth-form">
                        <select value={selectedQuestionId} onChange={e => setSelectedQuestionId((e.target as any).value)} required className="form-input">
                            <option value="">Select a question...</option>
                            {fallbackQuestions.map(q => <option key={q.id} value={q.id}>{q.question_text}</option>)}
                        </select>
                        <input type="text" placeholder="Your Answer" required value={fallbackAnswer} onChange={e => setFallbackAnswer((e.target as any).value)} />
                        ...
                        <div className="auth-step-footer">
                            <button type="button" className="text-link" onClick={() => setStep(2)}>
                                Remembered your code? <span>Back to Authenticator</span>
                            </button>
                        </div>
                    </form>
                )}

                {step === 1 && <p className="auth-footer">Don't have an account? <Link to="/register">Register</Link></p>}
            </div>
        </div>
    );
}
